...
async function fetchMovies(page = 1) {
  try {
    const res = await fetch(`${API}/api/movies?limit=40&page=${page}`, {
      next: { revalidate: 300 },
    });
  ...
  return (
    <>
      <div className="page-header">
        ...
        <p className="page-subtitle">共 {total} 部电影</p>
      </div>

      {items.length === 0 ? (
        <p className="empty-row">暂无电影内容</p>
      ) : (
      ...
      {totalPages > 1 && (
        <div className="pagination">
          <a href={page > 1 ? `/movies?page=${page - 1}` : undefined}>
            <button disabled={page <= 1}>上一页</button>
          </a>
          <span>第 {page} / {totalPages} 页</span>
          <a href={page < totalPages ? `/movies?page=${page + 1}` : undefined}>
